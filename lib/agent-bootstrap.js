...
const AGENT_BOOTSTRAP = Object.freeze({
  ...
  api_boundaries: {
    production: {
      ...
      credential_owner: 'The trusted local app backend process only; never the browser or coding-agent MCP configuration.',
      agent_access: 'wrapper_only'
    },
    local: {
  ...
  credential_setup: {
    ...
    credential_owner: 'SYM-Node profile integration store; injected only into declared app processes.',
    preferred_setup: 'Synchronize the profile-scoped Cosmise integration, then restart this managed app so SYM-Node injects COSMISE_MCP_TOKEN only into its backend process.',
    ...
    forbidden: [
      'Do not configure or call a separate production Cosmise MCP. Use only the installed app local MCP returned by Symposium.',
  ...
  credential_gate: {
    ...
    required_agent_steps: [
      ...
      'Restart cosmise-streamboards with the profile-scoped run_app tool so SYM-Node injects COSMISE_MCP_TOKEN only into the managed backend process.',
...
const AGENT_INSTRUCTIONS = [
  ...
  'After synchronization, restart cosmise-streamboards with the profile-scoped run_app tool so SYM-Node injects COSMISE_MCP_TOKEN only into its backend process.',
...
module.exports = { AGENT_BOOTSTRAP, AGENT_INSTRUCTIONS, CREDENTIAL_ENV };
