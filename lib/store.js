...
class AppStore {
  ...
  reconcileReports(boards = []) {
    ...
    this.state.reports = boards.map((board) => {
      ...
      return {
        ...
        public_url: isPublic === true ? text(board?.public_url, 2000) || previous.public_url || null : null,
  ...
  mergeReportUrls(streamboardId, input = {}) {
    ...
    if (report.is_public === true) report.public_url = text(input.public_url || input.share_url, 2000) || report.public_url || null;
  ...
  mergeReportPublication(streamboardId, input = {}) {
    ...
    report.public_url = input.is_public ? text(input.public_url, 2000) || report.public_url || null : null;
    report.url = report.public_url || report.edit_url || report.url || '';
    report.updated_at = now();
  ...
  mergeReportResolution(streamboardId, resolverOk) {
    ...
    report.verification = { ...(report.verification || {}), publication_ok: report.is_public === true, resolver_ok: resolverOk };
    report.updated_at = now();
