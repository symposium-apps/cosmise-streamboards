...
class AppStore {
  ...
  trim() {
    const active = (this.state.tasks || []).filter((task) => ['queued', 'running', 'waiting'].includes(task.status)).slice(0, MAX_TASKS);
    const terminal = (this.state.tasks || []).filter((task) => !['queued', 'running', 'waiting'].includes(task.status));
    this.state.tasks = [...active, ...terminal].slice(0, MAX_TASKS);
    this.state.events = (this.state.events || []).slice(0, MAX_EVENTS);
    this.state.reports = (this.state.reports || []).slice(0, MAX_REPORTS);
  }
  ...
  createTask(input = {}) {
    ...
    this.state.tasks = [task, ...this.state.tasks.filter((item) => item.id !== task.id)];
  ...
  upsertCallEvent(input = {}) {
    ...
    this.state.events = [existing, ...this.state.events.filter((event) => event.id !== existing.id)].slice(0, MAX_EVENTS);
    this.save('event');
    return existing;
  }
  ...
  addReport(input = {}) {
    const publicUrl = text(input.public_url, 2000) || (String(input.url || '').includes('/board/') ? text(input.url, 2000) : '');
    const report = {
    ...
    this.state.reports = [report, ...this.state.reports.filter((item) => item.id !== report.id)].slice(0, MAX_REPORTS);
  ...
  syncReports(inputs = []) {
    const existing = new Map(this.state.reports.map((report) => [report.id, report]));
    for (const input of inputs) {
      const id = text(input.id, 100) || text(input.streamboard_id, 100);
      if (!id) continue;
      const previous = existing.get(id) || {};
      const publicUrl = Object.prototype.hasOwnProperty.call(input, 'public_url') ? (text(input.public_url, 2000) || null) : (previous.public_url || null);
      existing.set(id, {
        ...previous,
        id,
        streamboard_id: text(input.streamboard_id, 100) || id,
        title: text(input.title, 200) || previous.title || 'Streamboard',
        url: publicUrl,
        public_url: publicUrl,
        edit_url: text(input.edit_url, 2000) || previous.edit_url || null,
        organisation: text(input.organisation, 160) || previous.organisation || null,
        description: text(input.description, 500) || previous.description || null,
        status: text(input.status, 40) || previous.status || (publicUrl ? 'ready' : 'private'),
        verification: input.verification && typeof input.verification === 'object' ? input.verification : previous.verification || null,
        updated_at: input.updated_at || previous.updated_at || now()
      });
    }
    this.state.reports = [...existing.values()].sort((left, right) => String(right.updated_at || '').localeCompare(String(left.updated_at || ''))).slice(0, MAX_REPORTS);
    return this.save('reports.synced');
  }
...
module.exports = { AppStore, TASK_STATUSES, EVENT_STATUSES, MAX_TASKS, MAX_EVENTS, MAX_REPORTS };
