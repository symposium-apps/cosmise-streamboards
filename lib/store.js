...
class AppStore {
  ...
  reconcileReports(boards = []) {
    ...
    this.state.reports = boards.map((board) => {
      ...
      const isViewable = isPublic === true || isProtected;
      return {
        id,
        streamboard_id: id,
        title: text(board?.name || board?.title, 200) || previous.title || 'Streamboard',
        url: text(board?.url || board?.edit_url, 2000) || previous.url || '',
        public_url: isViewable ? text(board?.public_url, 2000) || previous.public_url || null : null,
  ...
  mergeReportUrls(streamboardId, input = {}) {
    ...
    if (input.is_public === true) report.is_public = true;
    if (input.protected === true) report.protected = true;
    if (report.is_public === true || report.protected === true) report.public_url = text(input.public_url || input.share_url, 2000) || report.public_url || null;
  ...
  mergeReportPublication(streamboardId, input = {}) {
    ...
    report.public_url = input.is_public || report.protected ? text(input.public_url, 2000) || report.public_url || null : null;
    report.url = report.public_url || report.edit_url || report.url || '';
    report.updated_at = now();
  ...
  mergeReportResolution(streamboardId, resolverOk) {
    ...
    report.verification = { ...(report.verification || {}), publication_ok: report.is_public === true || report.protected === true, resolver_ok: resolverOk };
    report.updated_at = now();
