...
const { probeState, atomicWrite, RuntimeStateError } = require('./state-health');
...
class AppStore {
  ...
  load(fallback) {
    try {
      ...
      // Defaults are diagnostic-only when existing bytes could not be loaded.
      // Recovery requires a restart/reload, never replacement with defaults.
      this.loadFailed = error.code !== 'ENOENT';
      return fallback;
    }
  }
  ...
  snapshot() {
    ...
    state.sidebar_items = state.reports.map((report) => {
      ...
      const status = task?.status || report.status || 'ready';
      return {
        id,
        title: report.title,
        subtitle: report.description || report.organisation || 'Performance report',
        status,
        detail: task?.detail || report.description || null,
        selected: state.view.active_report_id === id,
        open: state.view.open_report_ids.includes(id),
        task_id: task?.id || null,
        updated_at: task?.updated_at || report.updated_at
      };
    });
    return state;
  }
  ...
  closeReport(streamboardId) {
    const id = text(streamboardId, 100);
    const index = this.state.view.open_report_ids.indexOf(id);
    const open = this.state.view.open_report_ids.filter((item) => item !== id);
    return this.setView({
      open_report_ids: open,
      active_report_id: this.state.view.active_report_id === id ? open[Math.max(0, index - 1)] || open[0] || null : this.state.view.active_report_id,
  ...
  interruptTasks(message) {
    for (const task of this.state.tasks) {
      if (['queued', 'running'].includes(task.status)) {
        task.status = 'failed'; task.detail = message; task.completed_at = now();
      }
    }
    for (const event of this.state.events) {
      if (event.status === 'running') { event.status = 'failed'; event.detail = message; }
    }
    if (['running', 'working'].includes(this.state.view.status)) this.state.view.status = 'failed';
  }

  runtimeFailure() {
    const error = new RuntimeStateError();
    this.state.runtime = { ...this.state.runtime, state_health: { writable: false, checked_at: now(), code: error.code } };
    this.interruptTasks(error.message);
    this.state.connection = { ...this.state.connection, state: 'error', message: error.message };
    return error;
  }

  checkHealth() {
    try {
      if (this.loadFailed) throw new RuntimeStateError();
      const health = probeState(this.file);
      this.state.runtime = { ...this.state.runtime, state_health: health };
      return health;
    } catch { throw this.runtimeFailure(); }
  }

  save(type = 'state') {
    try { this.checkHealth(); return this.persist(type); }
    catch { throw this.runtimeFailure(); }
  }
  ...
  updateConnection(input = {}) {
    const configured = this.state.runtime?.backend_mcp_configured;
    if (typeof configured === 'boolean') {
      input = { ...input, configured };
      if (!configured) input = { ...input, state: 'missing_key', message: 'Missing COSMISE_MCP_TOKEN in the app backend environment.' };
      else if (input.state === 'missing_key') input = { ...input, state: 'checking', message: 'Credential available; connection verification pending.' };
    }
  ...
  addReport(input = {}) {
    ...
    const report = {
      ...
      verification: input.verification && typeof input.verification === 'object' ? input.verification : null,
      is_public: input.is_public === true || Boolean(input.public_url),
  ...
  reconcileReports(boards = []) {
    ...
    this.state.reports = boards.map((board) => {
      ...
      const isPublic = typeof board?.is_public === 'boolean' ? board.is_public : typeof previous.is_public === 'boolean' ? previous.is_public : null;
      const isProtected = typeof board?.protected === 'boolean' ? board.protected : previous.protected === true;
      return {
        id,
        streamboard_id: id,
  ...
  mergeReportUrls(streamboardId, input = {}) {
    ...
    if (typeof input.is_public === 'boolean') report.is_public = input.is_public;
    if (typeof input.protected === 'boolean') report.protected = input.protected;
  ...
  mergeReportPublication(streamboardId, input = {}) {
    const id = text(streamboardId || input.streamboard_id || input.id, 100);
    if (!id || typeof input.is_public !== 'boolean') return null;
    let report = this.state.reports.find((item) => (item.streamboard_id || item.id) === id);
    if (!report) {
      report = { id, streamboard_id: id, title: text(input.title || input.name, 200) || 'Streamboard', status: 'ready', updated_at: now() };
      this.state.reports.unshift(report);
    }
    report.is_public = input.is_public;
    report.protected = input.protected === true;
    report.public_url = text(input.public_url, 2000) || report.public_url || null;
    report.url = report.public_url || report.edit_url || report.url || '';
    report.updated_at = now();
    this.save('report.publication');
    return report;
  }

  mergeReportResolution(streamboardId, resolverOk) {
    const id = text(streamboardId, 100);
    if (!id || typeof resolverOk !== 'boolean') return null;
    const report = this.state.reports.find((item) => (item.streamboard_id || item.id) === id);
    if (!report) return null;
    report.verification = { ...(report.verification || {}), publication_ok: resolverOk, resolver_ok: resolverOk, reported_is_public: report.is_public };
    report.updated_at = now();
    this.save('report.resolution');
    return report;
  }
