...
class CosmiseClient {
  ...
  applyResult(name, args, data, options = {}) {
    ...
    const createdBoardId = /^streamboards_create_(blank|from_template)$/.test(name) ? value.id : '';
  ...
  async reconcileReports() {
    ...
    try {
      ...
      for (const board of boards.slice(0, 20)) {
        ...
        try { await this.callTool('streamboards_get_urls', { streamboard_id: id }, { record: false, focus: false }); } catch { /* connection state carries safe error */ }
      }
    } finally {
      this.syncing = false;
    }
  ...
  start(intervalMs = 15000) {
    ...
    this.timer.unref?.();
    this.connectionTimer = setInterval(() => this.reconcile(), 60000);
