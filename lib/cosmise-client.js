...
const { TOKEN_NAME } = require('./profile-credential');
...
class CosmiseClient {
  constructor({ token, endpoint, store, catalog }) {
    ...
    this.browserClients = 0;
    this.pollIntervalMs = 300000;
    this.connectionIntervalMs = 900000;
  }
  ...
  async rpc(method, params = {}) {
    if (!this.token) throw new Error(`Missing ${TOKEN_NAME} in the app backend environment.`);
  ...
  async callTool(name, args = {}, options = {}) {
    ...
    try {
      ...
      if (tool.mode === 'write') {
        const refresh = setTimeout(() => this.reconcileReports().catch(() => {}), 250);
        refresh.unref?.();
      }
      return { tool: tool.name, data, mcp_result: mcpResult, duration_ms: Date.now() - started };
    } catch (error) {
      if (error.code === 'RUNTIME_STATE_UNWRITABLE') {
        const event = this.store.state.events.find(item => item.call_id === callId);
        if (event) { event.status = 'failed'; event.detail = error.message; }
        throw error;
      }
      const message = cleanError(this.token ? String(error?.message || error).split(this.token).join('[REDACTED]') : error);
      try {
      if (options.record !== false) this.store.upsertCallEvent({
        ...activity,
        status: 'failed',
      ...
      } catch (stateError) { throw stateError; }
      if (task) { this.store.interruptTasks(message); this.store.save('failure'); }
    ...
    try {
      ...
      for (const board of boards.slice(0, 20)) {
        ...
        try {
          ...
          const resolverOk = await this.publicResolverMatches(id, { ...state, ...(firstObject(urls.data) || {}) });
  ...
  async reconcile(options = {}) {
    ...
    try {
      ...
      if (error.code !== 'RUNTIME_STATE_UNWRITABLE') this.store.updateConnection({ configured: this.configured(), state: 'error', message: cleanError(error) });
      throw error;
    } finally {
      this.syncing = false;
    }
  ...
  start(intervalMs = 300000) {
    ...
    this.store.setRuntime({ ...this.store.state.runtime, backend_mcp_configured: this.configured(), credential_source: this.configured() ? 'app_process_environment' : 'missing', expected_env_var: TOKEN_NAME });
    this.store.interruptTasks('App restarted; previous work was interrupted. Verify its outcome before retrying.');
    this.store.state.connection = { ...this.store.state.connection, configured: this.configured(), state: this.configured() ? 'checking' : 'missing_key' };
    try {
      this.store.checkHealth();
      this.store.updateConnection({ configured: this.configured(), state: this.configured() ? 'checking' : 'missing_key', mode: null, organisation: null, endpoint: null, message: this.configured() ? 'Credential available; connection verification pending.' : `Missing ${TOKEN_NAME} in the app backend environment.` });
      this.store.checkHealth();
    } catch (error) { if (error.code !== 'RUNTIME_STATE_UNWRITABLE') throw error; }
  }

  connectBrowser(intervalMs = this.pollIntervalMs) {
    let released = false;
    if (!this.configured()) return () => {};
    this.browserClients += 1;
    if (this.browserClients === 1) {
      this.reconcile({ record: false }).catch(() => {});
      this.timer = setInterval(() => this.reconcileReports().catch(() => {}), intervalMs);
      this.timer.unref?.();
      this.connectionTimer = setInterval(() => this.reconcile({ record: false }).catch(() => {}), this.connectionIntervalMs);
      this.connectionTimer.unref?.();
    }
    return () => {
      if (released) return;
      released = true;
      this.browserClients = Math.max(0, this.browserClients - 1);
      if (this.browserClients > 0) return;
