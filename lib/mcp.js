...
function createMcp({ store, productionClient, catalog }) {
  ...
  async function handle(message) {
    ...
    try {
      if (message?.method === 'initialize') return { jsonrpc: '2.0', id, result: { protocolVersion: '2025-03-26', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'cosmise-streamboards-wrapper', version: '0.3.13' }, instructions: AGENT_INSTRUCTIONS } };
      ...
      return { jsonrpc: '2.0', id, result: textResult({ ok: false, code: error.code, error: error.message }, true) };
    }
  }
