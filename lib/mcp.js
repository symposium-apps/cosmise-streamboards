...
const AGENT_INSTRUCTIONS = 'When a Streamboard task is active, mirror every meaningful production Cosmise MCP call to cosmise_app_observe_call. Send one running update before the production call and one success or failed update with the same call_id afterwards. Summarize only what you are reading, building, or learning; never send credentials, headers, raw arguments, raw responses, or sensitive identifiers.';
...
const LOCAL_TOOLS = [
...

  {
    ...
    description: 'REQUIRED companion telemetry while a visible Streamboard task is active. Call once with status=running immediately before each meaningful production Cosmise MCP call, then again with the same call_id and status=success or failed. Send only a concise sanitized message and safe learned facts—never raw arguments, responses, credentials, headers, tokens, or sensitive identifiers.',
    inputSchema: {
      type: 'object',
      properties: {
        task_id: { type: 'string' },
        ...
        message: { type: 'string', description: 'Short user-safe description for the Building now bar.' },
...
function createMcp({ store }) {
  ...
  async function handle(message) {
    ...
    try {
      if (message?.method === 'initialize') return { jsonrpc: '2.0', id, result: { protocolVersion: '2025-03-26', capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'cosmise-streamboards-local', version: '0.1.0' }, instructions: AGENT_INSTRUCTIONS } };
...
module.exports = { AGENT_INSTRUCTIONS, createMcp, LOCAL_TOOLS };
