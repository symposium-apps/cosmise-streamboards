...
When `data.runtime.backend_mcp_configured` is false, the browser renders only the Cosmise connection gate. The backend reads `COSMISE_MCP_TOKEN` only from the managed app's own process environment. The safe recovery flow is: synchronize Cosmise so SYM-Node binds the app secret, restart the app, call `cosmise_app_sync_now`, and verify `streamboards_get_context`.
