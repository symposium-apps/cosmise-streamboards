...
When `data.runtime.backend_mcp_configured` is false, the browser renders only the Cosmise connection gate. The backend reads `COSMISE_MCP_TOKEN` only from its process environment. The safe recovery flow is: synchronize Cosmise, restart the app so SYM-Node injects the profile integration credential, call `cosmise_app_sync_now`, and verify `streamboards_get_context`.
